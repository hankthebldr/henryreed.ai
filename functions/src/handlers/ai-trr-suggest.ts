...
const AITRRSuggestRequest = z.object({
...
const AITRRSuggestResponse = z.object({
...
// Helper functions
const formatPrompt = (template: string, input: any): string => {
  return template.replace('{input}', JSON.stringify(input, null, 2));
};

const validateAndParseAIResponse = (response: string, suggestionType: string): any => {
  try {
    ...
    logger.error('Failed to parse AI response:', error, { response, suggestionType });
...
const logAIPrediction = async (
  ...
  try {
    ...
    logger.warn('Failed to log AI prediction:', error);
  }
};

// Main handler function
export const aiTRRSuggestHandler = async (
  data: unknown,
  ...
  logger.info('AI TRR Suggest request', {
    userId,
    trrId: request.trrId,
  ...
  try {
    ...
    logger.info('AI TRR Suggest completed', {
      userId,
      trrId: request.trrId,
      suggestionType: request.suggestionType,
