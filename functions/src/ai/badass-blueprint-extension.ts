...
const buildExecutiveSummary = (
  context: EngagementContextSnapshot,
  metrics: BadassBlueprintPayload['metrics'],
  theme: string
): BadassBlueprintSection => {
...
const buildTimelineSection = (context: EngagementContextSnapshot): BadassBlueprintSection => {
...
const buildScenarioSection = (context: EngagementContextSnapshot): BadassBlueprintSection => {
  const details = context.scenarios.map(
    scenario =>
      `${scenario.name} → ${scenario.status}. Impact: ${scenario.impact}. Highlights: ${scenario.highlights.join(', ')}`
  );

  return {
    title: 'Scenario Outcomes',
    summary: `${context.scenarios.length} POV scenarios executed covering ${
      context.metrics.coveragePercentage
    }% of the agreed capability footprint.`,
    details,
...
const buildRecommendationSection = (
  ...
  return {
    ...
    summary: `Prioritized actions to sustain momentum while mitigating residual risk (risk score ${
...
const buildNextStepsSection = (context: EngagementContextSnapshot): BadassBlueprintSection => {
...
const buildAppendixSection = (context: EngagementContextSnapshot): BadassBlueprintSection => {
...
export const generateBadassBlueprintPayload = async (
  context: EngagementContextSnapshot,
  emphasis: BadassBlueprintEmphasis | undefined,
  executiveTone: string | undefined
): Promise<BadassBlueprintPayload> => {
  ...
  const metrics: BadassBlueprintPayload['metrics'] = {
  ...
  const sections: BadassBlueprintSection[] = [
